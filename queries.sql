...
ORDER BY id;
/*More Queries.*/
SELECT COUNT(name) AS total_animals
FROM animals;
...
FROM animals;
SELECT neutered,
  SUM(escape_attempts) AS escape_Attempts
FROM animals
GROUP BY neutered
...
GROUP BY species;
SELECT species,
  ROUND(AVG(escape_attempts), 2) AS escape_attempts
FROM animals
WHERE EXTRACT(
    YEAR
    FROM date_of_birth
...
FROM owners
  JOIN animals ON animals.owners_id = owners.id
  JOIN species ON animals.species_id = species.id
WHERE owners.full_name = 'Jeniffer Orwell'
...
LIMIT 1;
SELECT vets.name AS vet,
  COUNT(animals.name) AS animals_seen
FROM visits
  JOIN vets ON vets_id = vets.id
  JOIN animals ON animals_id = animals.id
WHERE vets_id =(
...
SELECT animals.species_id AS species,
  visits.vets_id AS vet,
  specializations.species_id AS specialization
FROM visits
